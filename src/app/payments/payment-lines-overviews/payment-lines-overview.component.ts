...
export class PaymentLinesOverviewComponent {
  ...
  protected readonly _allPaymentLinesOverviewWithName = computed<readonly GetPaymentLinesOverviewWithName[]>(() => {
    ...
    return this.paymentLinesOverviews().map((paymentLineOverview) => ({
      ...paymentLineOverview,
      name: subCategoryIdTranslationsMap.get(paymentLineOverview.subCategoryId) || '',
    }));
  });

  protected readonly _paymentLinesOverviewWithName = computed<readonly GetPaymentLinesOverviewWithName[]>(() => {
    if (!this.paymentLinesOverviews()?.length) {
      return [];
    }
    ...
    for (let i = 0; i < this._numberOfItemsToShow(); i++) {
      paymentLinesOverviewWithName.push(this._allPaymentLinesOverviewWithName()[i]);
    }
  ...
  protected _numberOfItemsToShow = computed<number>(() => {
    return this._seeMore() ? this.paymentLinesOverviews().length : 3;
  });
}
